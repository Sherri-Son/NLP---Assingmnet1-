...
def _words_to_ids(words, word_embeddings):
    """Converts words to embedding indices; unseen words use the UNK index."""
    unk_idx = word_embeddings.word_indexer.index_of("UNK")
    ids = []
    for word in words:
        word_idx = word_embeddings.word_indexer.index_of(word)
        if word_idx == -1:
            word_idx = unk_idx
        ids.append(word_idx)
    return ids
